...
public sealed class ResetPasswordRequestHandler : ICommandHandler<ResetPasswordRequestCommand>
{
    ...
    private readonly string _resetPasswordTempalateId = "d-2e01bae7bcf7497a821c74528552c406";
    ...
    public async Task HandleAsync(ResetPasswordRequestCommand command, CancellationToken cancellationToken)
    {
        ...
        var linkToResetPassword = $"http://localhost:3000/reset-password?token={passwordResetToken.Token}&userId={passwordResetToken.UserId}";

        var templateData = new { url = linkToResetPassword };

        await _emailService.SendTemplateMailAsync(command.Email, _resetPasswordSubject, _resetPasswordTempalateId,
            templateData);
    }
}